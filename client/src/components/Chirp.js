...
//       <div>
//         {isAt ? (
//           <div className="atUser">{this.props.text}</div>
//         ) : (
//           <div className="userTrend">{this.props.text}</div>
//         )}
//       </div>
//     );
//   }
...
//     return <div>{isText ? text : <SpecialText type={type} text={text} />}</div>;
//   }
// }
...
const createShortcode = (type, entity, id) => {
  return `[${type}:${entity}]`;
};

const quickTest = (type, symbol, data) => {
  ...
  while (index >= 0) {
    ...
    let formattedString = createShortcode(type, subString, 1);
...
const splitMessage = content => {
  ...
  //   return content.filter(content, val => {
  //     return val && val !== '';
  //   });
  // }
  return content;
...
const parseChirp = chirp => {
  let data = quickTest('AT', '@', chirp);
  data = quickTest('TREND', '#', data);
...
class FormattedText extends React.Component {
  render() {
    return <React.Fragment>{formatChirp(this.props.text)}</React.Fragment>;
  }
}
